#!/usr/bin/env python3
"""AtomGuard data resolver.

只复用 AtomGuard 的 data/ 目录，不导入其扫描器执行逻辑。
...
AIG_REPO = "https://github.com/503496348-ops/atomguard.git"
DEFAULT_LOCAL_ROOTS = [
    Path("/Users/python/Downloads/sec-page/AtomGuard"),
    Path.cwd().parent / "AtomGuard",
]
...
def download_to_temp(repo: str = AIG_REPO, keep: bool = False) -> AIGData:
    ...
    root = temp_parent / "AtomGuard"
    cmd = ["git", "clone", "--depth", "1", repo, str(root)]
    try:
        subprocess.run(cmd, check=True)
    except FileNotFoundError as e:
        raise RuntimeError("git 不可用，无法下载 AtomGuard") from e
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"下载 AtomGuard 失败: {e}") from e
...
def main() -> None:
    ap = argparse.ArgumentParser(description="解析、下载或同步 AtomGuard data 目录")
    sub = ap.add_subparsers(dest="cmd", required=True)

    def add_common(p):
        p.add_argument("--aig-data-dir", help="AtomGuard/data 路径")
        p.add_argument("--aig-root", help="AtomGuard 仓库根目录")
