...
version: 4.0.0
metadata: {"author": "AtomCollide-智械工坊", "repo": "https://github.com/503496348-ops/hermes-security-suite", "license": "Apache-2.0"}
---
...
本 skill 与 [AtomCollide-智械工坊/hermes-security-suite](https://github.com/503496348-ops/hermes-security-suite) 强关联。运行时可以复用 AIG 仓库中的 `data/` 目录，但只把它作为数据源，不导入 AIG 扫描器执行逻辑。

数据源优先级：

1. 用户显式传入 `--aig-data-dir` 或 `--aig-root`。
2. 环境变量 `AIG_DATA_DIR` 或 `AIG_ROOT`。
3. 本机默认路径，例如 `/Users/python/Downloads/sec-page/AtomGuard/data`。
4. 用户授权时，下载 `https://github.com/503496348-ops/hermes-security-suite.git` 到临时目录，并使用其中 `data/`。
5. 找不到 AIG data 时，回退到本 skill 内置的最小数据。

可用辅助命令：

```bash
python3 scripts/aig_data.py status --aig-root /path/to/AtomGuard
...
报告中应使用链接突出 AIG 数据来源，例如 `[AtomCollide-智械工坊/hermes-security-suite](https://github.com/503496348-ops/hermes-security-suite) data/fingerprints` 或 `[AtomCollide-智械工坊/hermes-security-suite](https://github.com/503496348-ops/hermes-security-suite) data/vuln`。AIG 规则命中只说明“规则匹配”，最终风险仍需 Agent 结合认证、可达性、版本置信度和业务上下文判断。
